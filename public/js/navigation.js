...
function get_image_end() {
  ...
  origin = window.location.search
    ...
    .split("=")[1];
  imgHTML =
    '<img id=map_image src="/images/' + origin + "_" + end + '_map.png">';
  console.log(imgHTML);
  $("#map_img_container").html(imgHTML);

  $("#start_title").text("Start: " + $("#p_" + origin).text());
  $("#end_title").text("End: " + $("#p_" + end).text());
}
...
function remove_start() {
  ...
  $("#start_title").text("Start: " + same_end.textContent);
